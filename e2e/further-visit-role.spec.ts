 ...
 * 2. SR", so an N4 is offered the 1. SR alone.
 */
...
test('an N3 is offered a further visit in either role, and the choice is kept', async ({ page }) => {
  ...
  await expect(answers(page)).toHaveText([/^Y, (as|als) 1SR$/, /^Y, (as|als) 2SR$/, 'N']);
...
test('an N4 is offered the 1. SR only', async ({ page }) => {
  ...
  await expect(answers(page)).toHaveText([/^Y, (as|als) 1SR$/, 'N']);
});
