...
test.describe('admin console', () => {
  ...
  test('Spiele shows the settings season on the tab, test games exempt — and nothing before it knows the season', async ({ page }) => {
    ...
    await page.route('**/api/eligible-games*', (r) => r.fulfill({ json: [
      { ...GAME, id: 'g-now', homeTeam: 'VBC Jetzt', date: `${SEASON}-10-10T18:00:00Z`, assignedRc: '' },
      // The season before's final — the row from the screenshot, moved along.
      { ...GAME, id: 'g-last', homeTeam: 'VBC Damals', date: `${SEASON}-03-21T13:30:00Z`, assignedRc: '' },
      // Made in the summer, in no season at all: shown, as everywhere else.
      { ...GAME, id: 'g-test', homeTeam: 'VBC Probe', date: `${SEASON}-07-01T18:00:00Z`, assignedRc: '', isManual: true },
    ] }));
...
test.describe('coach app', () => {
  ...
  test('a link to last season\'s game is "not found", not a blank form', async ({ page }) => {
    ...
    await page.route('**/api/eligible-games*', (r) => r.fulfill({ json: [
      GAME,
      { ...GAME, id: 'g-last', homeTeam: 'VBC Damals', date: '2026-03-21T13:30:00Z' },
    ] }));
    await page.goto('/form/g-last/1sr');
  ...
  test('a link to last season\'s coachee row is "not found" too', async ({ page }) => {
    await stubSignedInApp(page);
    ...
    await expect(page.getByText(/Coachee not found|Coachee nicht gefunden/)).toBeVisible();
