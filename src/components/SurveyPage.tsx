  ...
  const Both = ({ entry, className, enClassName, block }: {
    ...
    return (
      <>
        ...
        <span className={cn(block ? 'block' : '', enClassName ?? 'text-stone-400')}>{en}</span>
      </>
    );
  };
      ...
      <div className="w-full max-w-xl mt-6 mb-10">
        ...
        {state === 'error' && (
          <div className={`${card} p-6 text-center`}>
            <p className="text-sm font-medium text-red-600"><Both entry={SURVEY_UI.errorTitle} block enClassName="block text-stone-400 mt-1" /></p>
            <p className="text-xs text-stone-400 mt-1.5"><Both entry={SURVEY_UI.errorBody} block enClassName="block text-stone-400 mt-1" /></p>
          </div>
        )}
        ...
        {state === 'already' && (
          <div className={`${card} p-8 text-center`}>
            ...
            <p className="text-sm font-medium text-stone-800"><Both entry={SURVEY_UI.alreadyTitle} block enClassName="block text-stone-400 mt-1" /></p>
            <p className="text-xs text-stone-400 mt-1.5"><Both entry={SURVEY_UI.alreadyBody} block enClassName="block text-stone-400 mt-1" /></p>
          </div>
        )}
        ...
        {state === 'done' && (
          <div className={`${card} p-8 text-center`}>
            ...
            <p className="text-sm font-medium text-stone-800"><Both entry={SURVEY_UI.thanksTitle} block enClassName="block text-stone-400 mt-1" /></p>
            <p className="text-xs text-stone-400 mt-1.5"><Both entry={SURVEY_UI.thanksBody} block enClassName="block text-stone-400 mt-1" /></p>
          </div>
        )}
          ...
          <div className="flex flex-col gap-4">
            <div className={`${card} p-5`}>
              ...
              {bothLangs(form.intro).en && (
                <p className="text-sm text-stone-400 leading-relaxed whitespace-pre-line mt-3 pt-3 border-t border-stone-100">{bothLangs(form.intro).en}</p>
              )}
            </div>
            ...
            <div className={`${card} p-5`}>
              ...
              <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-sm">
                <dt className="text-stone-400"><Both entry={SURVEY_UI.fieldReferee} enClassName="text-stone-300" /></dt>
                <dd className={anonymous ? 'text-stone-300 line-through' : 'text-stone-800 font-medium'}>{visit.referee}</dd>
                <dt className="text-stone-400"><Both entry={SURVEY_UI.fieldDate} enClassName="text-stone-300" /></dt>
                <dd className="text-stone-800">{visit.date}</dd>
                <dt className="text-stone-400"><Both entry={SURVEY_UI.fieldMatchNo} enClassName="text-stone-300" /></dt>
                <dd className="text-stone-800">{visit.matchNo}</dd>
                <dt className="text-stone-400"><Both entry={SURVEY_UI.fieldRc} enClassName="text-stone-300" /></dt>
              ...
              <label className="flex items-start gap-3 mt-4 pt-4 border-t border-stone-100 cursor-pointer">
                ...
                <span>
                  ...
                  <span className="block text-xs text-stone-400 mt-0.5 leading-snug"><Both entry={SURVEY_UI.anonHelp} block enClassName="block text-stone-300 mt-0.5" /></span>
                </span>
              </label>
            </div>
            ...
            {form.questions.map((q) => {
              ...
              return (
                ...
                <div key={q.id} data-log-redact className={`${card} p-5`}>
                  ...
                  {bothLangs(q).en && <p className="text-sm text-stone-400 leading-snug">{bothLangs(q).en}</p>}
                  {hint.de && <p className="text-xs text-stone-400 mt-1 leading-snug">{hint.de}</p>}
                  {hint.en && <p className="text-xs text-stone-300 leading-snug">{hint.en}</p>}
                  {q.kind === 'choice' ? (
                    <div className="flex flex-col gap-1.5 mt-3">
                      {optionsOf(q).map((o) => (
                        <label key={o.value} className="flex items-center gap-2.5 cursor-pointer group">
                          ...
                          />
                          <span className="text-sm text-stone-600 group-hover:text-stone-900"><Both entry={o} enClassName="text-stone-400" /></span>
                        </label>
                      ))}
                    </div>
                  ) : (
